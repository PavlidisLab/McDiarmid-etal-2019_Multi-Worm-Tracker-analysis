...
set.seed(config::get("tsne")$seed2D)
...
cairo_pdf(
  filename = paste0(config::get("tsne")$`output-directory`, "2D-tSNE.ASD.pdf"),
  onefile = T,
  width = 9,
  height = 5.5
)
...
p <- ggplot(tsne_Y.ASD) +
  ...
  labs(
    title = paste("t-SNE of t-test values for", cluster.string, "genes") ,
    subtitle = "",
    x = "x",
    y = "y"
  )  +
...
cairo_pdf(
  filename = paste0(config::get("tsne")$`output-directory`, "2D-tSNE_ASD_labelled.pdf"),
  onefile = T,
  width = 19,
  height = 11
)
...
cairo_pdf(
  filename = paste0(config::get("tsne")$`output-directory`, "3D-tSNE_ASD.pdf"),
  onefile = T,
  height = 12,
  width = 12
)
scatter3D(
  ...
  y = tsne_Y.3d[, 2],
  z = tsne_Y.3d[, 3],
  col = "dodgerblue2", # N2 wild-type colored manually post-hoc.
  phi = 0,
  bty = "g",
  theta = 30,
  pch = 20,
