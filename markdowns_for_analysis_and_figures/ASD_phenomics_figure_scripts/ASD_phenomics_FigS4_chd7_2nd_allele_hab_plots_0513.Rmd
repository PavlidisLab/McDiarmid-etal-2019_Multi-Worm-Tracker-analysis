---
title: "CHD8 2nd Allele Habituation"
...
setwd("/Volumes/SAMWISE/Troy/ASD_Phenomics_pub_data/VC606_N2_0129_2019/output")
...
cleanrawsrev$genotype <- recode(cleanrawsrev$genotype, `N2` = "Wild-type (N2)", `VC606` = "chd-7")
...
cleanrawsrev$genotype <- factor(cleanrawsrev$genotype,
    levels = c('Wild-type (N2)', 'chd-7',  ordered = TRUE))
...
```{r}
##Plot of mean reversal probability for chd-7 2nd allele

mean_cleanrawsrev_filt <- mean_cleanrawsrev %>% 
  filter(genotype %in% c("Wild-type (N2)", "chd-7"))
...
ggplot(mean_cleanrawsrev_filt, aes(stimulus_num, rev_prob, colour = genotype)) +
  ...
  scale_color_manual(values=c("black", "gray39", "dodgerblue2")) +
  ...
  ggsave("chd-7 2nd allele.jpeg", width = 7, height = 6, path = "/Users/troymcdiarmid/Desktop/ASD_Screen_materials")
...
data <- read.table("/Volumes/SAMWISE/Troy/ASD_Phenomics_pub_data/VC606_N2_0129_2019/output/data.srev", sep="", header = FALSE)
...
write.table(my_tukey_summary, "/Volumes/SAMWISE/Troy/ASD_Phenomics_pub_data/VC606_N2_0129_2019/output/stats.csv")
